...
(function () {
  ...
  var CDN = 'https://cdn.jsdelivr.net/gh/makemoneytrading/mmt-waitlist-assets@a3090bad48ecbe98b7bab1e5d5f7337b885c165d/homepage-v3/';
...
(function(){
  ...
  var half = Math.ceil(videos.length / 2); // 7 up top, 7 below
...
(function(){
  ...
  var outWeek  = document.getElementById('i-week');
  var outMonth = document.getElementById('i-month');
  var outAnn   = document.getElementById('i-annual');
  if (!inYear) return;

  var DAYS_PER_YEAR = 240; // 20 trading days x 12 months
  function compute(){
    var salary = _parseNum(inYear.value);
    var daily = salary / DAYS_PER_YEAR;
    var week  = salary / 52;
    var month = salary / 12;
    if (outDaily) outDaily.innerHTML = _fmt$(daily) + '<span class="c2r-unit">/ day</span>';
    if (outWeek)  outWeek.textContent  = _fmt$(week);
    if (outMonth) outMonth.textContent = _fmt$(month);
    if (outAnn)   outAnn.textContent   = _fmt$(salary);
  }
